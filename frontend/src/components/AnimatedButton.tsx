...
const AnimatedButton: React.FC<AnimatedButtonProps> = ({ 
  ...
  return (
    <div className={`animated-btn-wrapper ${className}`}>
      <button 
        className={`animated-btn ${isLoading ? 'loading' : ''}`} 
        ...
        <div className="state state--default">
          ...
          <p>
            {text.split('').map((char, idx) => (
              <span key={idx} style={{ '--i': idx } as React.CSSProperties}>{char === ' ' ? '\u00A0' : char}</span>
            ))}
          </p>
        </div>
        <div className="state state--sent">
          ...
          <p>
            {loadingText.split('').map((char, idx) => (
              <span key={idx} style={{ '--i': idx + text.length } as React.CSSProperties}>{char === ' ' ? '\u00A0' : char}</span>
            ))}
          </p>
        </div>
      </button>
